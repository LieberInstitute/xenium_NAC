...
# Load the final (second-pass) Spateo coordinates for Br6436
csv_path = (
    git_root
    / "processed-data"
    / "05_Xenium_alignment"
    / "module02_Spateo_second_pass_Br6436"
    / "Br6436_second_pass_coordinates.csv"
)
...
adata_Br6436.obs["cell_id"] = adata_Br6436.obs["cell_id"].astype(str)
...
missing = set(adata_Br6436.obs["cell_id"]) - set(coord_map.index)
if missing:
    raise ValueError(
        f"{len(missing)} Br6436 cells were not found in the second-pass "
        "coordinate CSV"
    )
# Reorder aligned coordinates to match adata_Br6436.obs cell order
matched = coord_map.loc[adata_Br6436.obs["cell_id"]]
adata_Br6436.obsm["align_spatial"] = matched[
    ["x_second_pass", "y_second_pass"]
].to_numpy()
adata_Br6436.obs["z_height"] = matched["z_height"].to_numpy()
...
plot_consecutive_overlays(
    aligned_slices,
    key_added,
    plot_outdir / "Br6436_spateo_sanity_check.png",
)
...
plt.xlabel("x")
plt.ylabel("y")
plt.title("Overlaid XY coordinates of Br6436 slices")
plt.savefig(
    plot_outdir / "Br6436_XY.png",
    dpi=300,
    bbox_inches="tight",
)
plt.close("all")
...
plt.savefig(
    plot_outdir / "Br6436_slice_grid_overlay.png",
    dpi=300,
    bbox_inches="tight",
)
plt.close("all")
...
def plot_grid_props_plane(plane="xy"):
    ...
    plt.savefig(
        plot_outdir / f"Br6436_cell_type_proportion_grid_{plane}.png", # name according to axis
        dpi=300,
        bbox_inches="tight",
    )
    plt.close("all")
...
sample_key = "Sample"
selected_sample = "Br6436_Nac_7_3660"   # change this to the representative slice we want
pt_size = 0.25
pt_alpha = 0.7
...
plt.savefig(
    plot_outdir / f"Br6436_grid_{grid_i + 1}_{grid_j + 1}_proportion_by_z_horizontal.png",
    dpi=600,
    bbox_inches="tight",
)
plt.close("all")
