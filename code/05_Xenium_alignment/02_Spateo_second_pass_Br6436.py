...
MODULE_NAME = "module02_Spateo_second_pass_Br6436"
...
# Subset to Br6436 sample
DONOR = "Br6436"
adata_Br6436 = adata_all[adata_all.obs["Donor"] == DONOR].copy()
adata_Br6436.obs = adata_Br6436.obs.reset_index(drop=True)
adata_Br6436.obsm['spatial'] = np.asarray(getattr(adata_Br6436.obsm['spatial'], "values", adata_Br6436.obsm['spatial']))

sample_str = adata_Br6436.obs["Sample"].astype(str)
adata_Br6436.obs["z_height"] = (
    sample_str.str.extract(r'_(\d+)$', expand=False)
              .pipe(pd.to_numeric, errors="coerce")
)

s = adata_Br6436.obs["Sample"]
...
observed_samples = set(adata_Br6436.obs["Sample"].dropna().astype(str))
missing_samples = [sample for sample in SAMPLE_ORDER if sample not in observed_samples]
if missing_samples:
    raise ValueError(f"Missing Br6436 samples: {missing_samples}")

slices = [
    adata_Br6436[adata_Br6436.obs["Sample"].astype(str) == sample].copy()
...
transformation_path = (
    ...
    / "processed-data"
    / "05_Xenium_alignment"
    / "module01_Spateo_initial_pass_Br6436"
    / "Spateo_transformation_Br6436.pkl"
)
...
def realign_pair(
    ...
    use_celltype_labels=True,
):
    """Recalculate one consecutive-pair transform and replace it in place."""
    if pair_index < 0 or pair_index >= len(slices) - 1:
        raise ValueError(
            f"pair_index must be between 0 and {len(slices) - 2}; "
            f"received {pair_index}"
        )
    keep_celltypes = set(keep_celltypes)
    if not keep_celltypes:
        raise ValueError("keep_celltypes cannot be empty")

    reference = slices[pair_index].copy()
    moving = slices[pair_index + 1].copy()
    reference_sub = reference[
        reference.obs["CellType"].astype(str).isin(keep_celltypes)
    ].copy()
    moving_sub = moving[
        moving.obs["CellType"].astype(str).isin(keep_celltypes)
    ].copy()
    if reference_sub.n_obs == 0 or moving_sub.n_obs == 0:
        raise ValueError(
            "Selected cell types must occur in both slices for pair "
            f"{pair_index}: {SAMPLE_ORDER[pair_index]} -> "
            f"{SAMPLE_ORDER[pair_index + 1]}"
        )

    alignment_kwargs = {}
    if use_celltype_labels:
        reference_sub.obs["CellType"] = reference_sub.obs["CellType"].astype(
            "category"
        )
        moving_sub.obs["CellType"] = moving_sub.obs["CellType"].astype(
            "category"
        )
    ...
    print(
        f"Re-aligning pair {pair_index}: {SAMPLE_ORDER[pair_index]} -> "
        f"{SAMPLE_ORDER[pair_index + 1]}; cell types={keep_celltypes}; "
        f"use_celltype_labels={use_celltype_labels}"
    )
    current_transformation = st.align.morpho_align_transformation(
        models=[reference_sub, moving_sub],
        spatial_key=spatial_key,
        key_added=key_added,
        device=device,
        verbose=True,
    ...
    transformation[pair_index] = current_transformation[0]

### Re-alignment plan for selected pairs of slices
# ============================================================================
REALIGNMENT_PLAN = [
    dict(
        pair_index=0,
        keep_celltypes={"MSN_Oligo"},
        partial_robust_level=100,
        use_celltype_labels=True,
    ),
    dict(
        pair_index=1,
        keep_celltypes={"CHAT", "Excitatory"},
        partial_robust_level=100,
        use_celltype_labels=True,
    ),
    dict(
        pair_index=3,
        keep_celltypes={"Astro_A", "Astro_B"},
        partial_robust_level=100,
        use_celltype_labels=True,
    ),
    dict(
    ...
    dict(
        pair_index=7,
        keep_celltypes={"CHAT", "Excitatory", "Astro_A"},
        partial_robust_level=100,
        use_celltype_labels=True,
    ),
    dict(
...
planned_pair_indices = [entry["pair_index"] for entry in REALIGNMENT_PLAN]
if len(planned_pair_indices) != len(set(planned_pair_indices)):
    raise ValueError("REALIGNMENT_PLAN contains a duplicate pair_index")
for realignment_settings in REALIGNMENT_PLAN:
    realign_pair(**realignment_settings)
...
plot_consecutive_overlays(
    aligned_slices,
    key_added,
    PLOT_DIR / "Br6436_overlay_slices_2d_second_spateo.png",
...
save_path = OUTPUT_DIR / "Spateo_transformation_Br6436.pkl"
with open(save_path, "wb") as f:
    pickle.dump(transformation, f, protocol=pickle.HIGHEST_PROTOCOL)

import anndata as ad
aligned_adata = ad.concat(aligned_slices)
pd.unique(aligned_adata.obs["z_height"]) # array([ 650, 1150, 1650, 2150, 2650, 3150, 3660, 4150, 4650, 5150, 5650])
...
csv_path = OUTPUT_DIR / "Br6436_second_pass_coordinates.csv"
...
export_interactive_html(
    point_cloud,
    PLOT_DIR / "Br6436_reconstruction_3d_second_spateo.html",
    camera_position="iso",
)
...
export_interactive_html(
    point_cloud_flipped,
    PLOT_DIR / "Br6436_reconstruction_3d_second_spateo_flipped_z.html",
    camera_position="iso",
)
