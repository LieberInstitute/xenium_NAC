...
group <- paste0(sfe$sample_id, "_", sfe$spatial_0.4)
...
cell_idx <- splitit(sfe_sub$spatial_0.4)
...
pdf(file = here("plots","HD_Full_Analysis",
                "Banksy_sr","spatial_0.4_Expression_heatmap.pdf"),height = 12, width = 18)
draw(hm)
dev.off()
...
#Run mean ratio
gmr <- get_mean_ratio(sfe,
                      cellType_col = "spatial_0.4",
                      assay_name = "logcounts")

save(gmr,
     file = here("processed-data","HD_Full_Analysis","Banksy_sr","spatial_0.4_getmeanratio.rda"))
...
mod <- with(colData(sfe), model.matrix(~ sample_id))
...
save(markers_pairwise,
     file = here("processed-data","HD_Full_Analysis","Banksy_sr","spatial_0.4_pairwise.rda"))
...
save(markers_1vALL_enrich,file = here("processed-data","HD_Full_Analysis","Banksy_sr","spatial_0.4_1vALL.rda"))
