...
spe <- loadHDF5SummarizedExperiment(here(
  ...
  "spe_banksy_0.8_cell_level_noAGF"
))
...
for(clust_col in clust_cols){
  ...
  saveRDS(object = cluster_colors[[clust_col]],
          file = here("processed-data","HD_Full_Analysis","Cluster_colors",paste(clust_col,"_cell_level_spatial_noAGF_colors.Rds")))
}
...
for(sample in samples){
  ...
  for(clust_col in clust_cols){
    ...
    ggsave(
      here("plots","HD_Full_Analysis","Banksy","Cell_Level","Spatial","NoAGF",paste0(sample,"_",clust_col,".png")), 
      p,
