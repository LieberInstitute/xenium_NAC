...
export MPLCONFIGDIR="${TMPDIR:-/tmp}/matplotlib-crawdad-z-score-${USER}"

"${PYTHON}" "${SCRIPT_DIR}/04_crawdad_z_score.py" \
  --input-root \
  ...
  --output-dir \
    "${PROJECT_ROOT}/../processed-data/10_Xenium_CRAWDAD/module04_crawdad_z_score" \
  --plot-dir \
    "${PROJECT_ROOT}/../plots/10_Xenium_CRAWDAD/module04_crawdad_z_score" \
  --overwrite \
  "$@"

# Full 20 × 20 run with four region panels:
# bash run_04_crawdad_z_score_Br6660.sh
#
