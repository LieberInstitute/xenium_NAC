#Goal: Run Banksy spatial clustering with lambda values of 0.2 and 1.0  and several different k_geom values
...
#Set lambda and k_geom
args <- commandArgs(trailingOnly=TRUE)
lambda <- as.numeric(args[[1]])
k_geom <- as.integer(args[[2]])

message(paste0("Running BANKSY with lambda= ",lambda,",","kgeom=",k_geom," - ",Sys.time()))
...
spe_joint <- clusterBanksy(spe_joint, use_agf = use_agf, lambda = lambda, resolution = res, algo = "leiden",seed = 1000)
...
head(cluster_assign)
write.csv(cluster_assign,here("processed-data","05_Clustering",paste0("Banksy_leiden_","lambda",lambda,"kgeom",k_geom,"_clusters.csv")))

#Save the object. 
message(paste0("Saving object containing Banksy embedding - ",Sys.time()))
saveHDF5SummarizedExperiment(x = spe,
                             dir = here("processed-data","05_Clustering","SPEs",
                                        paste0("spe","_lambda",lambda,"_kgeom",k_geom)),
                             replace = TRUE,
                             verbose = TRUE,
                             as.sparse = TRUE)
