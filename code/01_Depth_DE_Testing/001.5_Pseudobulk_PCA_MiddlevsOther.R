...
#Group anterior/posterior samples together. 
Ant_Mid_Post$Depth <- ifelse(Ant_Mid_Post$Depth == "Middle","Middle","Anterior_Posterior")
...
sce$Depth <- factor(x = sce$Depth,levels=c("Anterior_Posterior","Middle"))
...
ggsave(plot = celltype_plot,
       filename = paste0("/dcs05/lieber/marmaypag/xenium_NAC_LIBD4125/xenium_NAC/plots/01_Depth_DE_Testing/PCA_Plots/Cell_Type_Specific/Middle_vs_Other/",celltype_run,"_HDGs_Depth_2Levels_PCA.pdf"),
       height = 12,
       width = 12)
...
dir_name <- paste0("/dcs05/lieber/marmaypag/xenium_NAC_LIBD4125/xenium_NAC/plots/01_Depth_DE_Testing/Regression/Middle_vs_Other/",celltype_run)
dir.create(dir_name)
...
saveRDS(stat_df,file = here("processed-data","Regression_stats","MiddlevsOther",paste0(celltype_run,"MiddlevsOther_Stats.Rds")))
