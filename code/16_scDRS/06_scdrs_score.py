"""Compute scDRS disease scores for one GWAS trait on Visium-HD cells.

Input: VHD_sfe_counts.h5ad (439k SpaceRanger-segmented cells; obs carries
`Spatial_Domain` and `labels` [snRNA-transferred cell types]).

Covariates: intercept, log(n_genes detected), and Sample indicator variables
(absorbs sample/donor batch).
...
OUT_DIR = BASE / "processed-data/16_scDRS/scores"
N_CTRL = 1000
...
def main(trait_key: str) -> None:
    ...
    # scDRS expects size-factor-normalized, log1p data
