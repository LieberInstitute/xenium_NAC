...
#SBATCH --output=logs/NAC-spaceranger-2603_missingSlideID-%a.txt
#SBATCH --array=1-2
...
## Locate file
SAMPLE=$(awk 'BEGIN {FS="\t"} {print $1}' 04-sample_ids-2603_slideIDmissing.txt | awk "NR==${SLURM_ARRAY_TASK_ID}")
IMAGE=$(awk 'BEGIN {FS="\t"} {print $2}' 04-sample_ids-2603_slideIDmissing.txt | awk "NR==${SLURM_ARRAY_TASK_ID}")
IMGCYT=$(awk 'BEGIN {FS="\t"} {print $3}' 04-sample_ids-2603_slideIDmissing.txt | awk "NR==${SLURM_ARRAY_TASK_ID}")
SAMPARG=$(awk 'BEGIN {FS="\t"} {print $4}' 04-sample_ids-2603_slideIDmissing.txt | awk "NR==${SLURM_ARRAY_TASK_ID}")
